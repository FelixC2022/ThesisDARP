...
#PRECOMPUTING DISTANCES BETWEEN VERTICES; NOT ACCOUNTING YET FOR DIFFERENT DEPOTS 

total_vertices = (num_reqs*2)+2         

t= np.zeros(shape=(total_vertices,total_vertices)) #t array contains travel time = distance of each arc. 

for i in range(total_vertices):
    for j in range(total_vertices):
        t[i][j]=np.linalg.norm((np.array((x_co[i],y_co[i]))) - (np.array((x_co[j],y_co[j])))) #euclidean distance


#SOME HELPER FUNCTIONS USED 

#distance from depot route/vehicle k to vertex j  
def dist(k,j):
    return np.linalg.norm((depot_co[k]) - (np.array((x_co[j],y_co[j])))) 


def compute_costincrease(k,a,tour, i,j): #tour=(..., i, ..., j,...), newtour=(..., a, i, ..., a+n, j,...)
    addcost=0
    n=num_reqs
    
    #1ND CONDITION 
    if tour[i-1]!=0 and tour[j]!=(2*n+1): #MDHDARP -> In this case all the correct distances are stored in array t
        addcost= -t[tour[i-1]][tour[i]] + t[tour[i-1]][a] + t[a+n][tour[j]]
        
    elif tour[i-1]==0 and tour[j]!=(2*n+1): #MDHDARP -> tour[i-1] is artificial start depot. Dist function needs to be called
        addcost= -dist(k, tour[i]) + dist(k, a) + t[a+n][tour[j]]
        
    elif tour[i-1]!=0 and tour[j]==(2*n+1): #MDHDARP -> tour[j] is end depot, dist fct needed for correct distance.
        
        if i!=j: 
            addcost= -t[tour[i-1]][tour[i]] + t[tour[i-1]][a] + dist(k, a+n)
        
        else: #i==j: this corresponds to adding a+n directly after a, and just before the end depot. 
            addcost= -dist(k,i-1) + t[tour[i-1]][a] + dist(k, a+n)
            
    elif tour[i-1]==0 and tour[j]==(2*n+1): #MDHDARP -> similar explanation as above.
        if i!=j:
            addcost= -dist(k, tour[i]) + dist(k, a) + dist(k, a+n)
        else: #i==j: this corresponds to adding a+n directly after a in an empty tour. (-dist(k, tour[i]) = 0)
            addcost= dist(k, a) + dist(k, a+n)
            
            
    #2ND CONDITION        
    if i!=j: #if a+n not directly placed after a
        if tour[j]!=(2*n+1):
            addcost += (-t[tour[j-1]][tour[j]] +t[a][tour[i]] + t[tour[j-1]][a+n])
        elif tour[j]==(2*n+1):
            addcost += (-dist(k, tour[j-1]) +t[a][tour[i]] + t[tour[j-1]][a+n])
    else: #if a+n directly placed after a (i==j)
        addcost += t[a][a+n]
    return addcost


def length_tour(tour, k):
    from_depot = dist(k, tour[1])
    to_depot = dist(k, tour[-2])
    other_distances = [t[tour[i]][tour[i+1]] for i in range(1,len(tour)-2)]
    total_distance = from_depot + to_depot + sum(other_distances)
...
#FC: an action is thus defined as the next user whose pickup and delivery vertex will be included in the solution 
#For this user we only consider the best insertion indexes, number of legal actions is the equal to the number of unserved actions 
def get_legal_actions(state): 
    unserved_requests= []
    pre = state[0] #state[0] is the succ array
...
#not a member function of the node class.
...
#state[0] contains the succ array. Those arrays will be initialized at None, and as long as there's a vertex with None in the
#succ array, there are still unserved requests. Note that this could also be done with the pre array (=state[1]).
#Also, we also have to loop through the user pickup vertices positions of the succ array, because our move definition 
#guarantees pairing and precedence constraints, and hence, only checking half of the array suffies. 
...
def move(state, a):
    ...
    for k in vehicles:
        ...
        for i in range(1, len(tour)): 
            ...
            for j in range(i+1, len(tour)): #NOTE: in case route k is empty, len([0,2n+1])=2 -> if i==1, this j loop will
                ...
                earliest_j = max(e[a+n], earliest_j + s[tour[j-1]] + t[tour[j-1]][a+n]) #incremental update earliest for vertex
                                                                                        #a+n, if inserted at location j.
                if earliest_j > l[a+n]:
                    break
    ...
    earliest_child[a]=max(e[a],earliest[pre_child[a-1]] + s[pre_child[a-1]] + t[pre_child[a-1]][a]) #what if a is the first one in the route 
    ...
    while vtx1 != 2*n+1: #can be 2*n+1 (end_depot)
        ...
        if vtx1 != 2*n+1: 
            earliest_child[vtx1] = max(e[vtx1],earliest_child[pre_child[vtx1-1]] + s[pre_child[vtx1-1]] + t[pre_child[vtx1-1]][vtx1])
            for r in R: 
                cap_child[r][vtx1] = cap[r][pre_child[vtx1-1]] + q[vtx1][r]
        else: #in this case vtx = end_depot (241) and hence pre_child[vtx1-1] thus not exist (index from 0 - (240-1))
            earliest_child[vtx1] = max(e[vtx1], earliest_child[RI_child[k][1]] + s[RI_child[k][1]] + dist(k, RI_child[k][1]))
            cap_child[r][vtx1] = cap[r][RI_child[k][1]] + q[vtx1][r]    
    
    while vtx2 != pre_child[RI_child[k][0]-1]:
        vtx2=pre_child[vtx2-1]
        if vtx2 != 0: #startdepot 
            latest_child[vtx2]=min(l[vtx2], latest_child[succ_child[vtx2-1]] - s[a+n] - t[a+n][succ_child[a+n-1]])
        else: #in case vtx2 has become the start depot 
            latest_child[vtx2] = min(l[vtx2], latest_child[RI_child[k][0]] + s[RI_child[k][0]] + dist(k, RI_child[k][0]))
...
class MonteCarloTreeSearchNode():
    def __init__(self, state, parent=None, parent_action=None):
        ...
        #earliest, latest and cap are stored in the state variable of each node, for each vertex, and the values depend on the current partial solution
        #which is stored in 'state'. Those variables are needed for preliminary feasibility checks as described in Braekers et
        #al., 2014. 
        
        #FC: I think the values can easily calculated this way 
        #self._earliest = state[3]
        #self._latest = state[4]
        #self._cap = state[5]
        
        #BW: 
        #self._earliest=[None]*len(V)   #V the set of vertices
        #self._earliest=self.compute_earliest()
        #self._latest=[None]*len(V)
        #self._latest=self.compute_latest()
        #self._cap=[[None for v in range(len(V))] for r in range(len(R))]    #initializes a 2D list of |R| lists with length |V| 
        #for r in R: #assuming that the resources are encoded in list R, as integers. For example r==0 -> regular seat.
            #self._cap[r]= self._compute_cap(r)
    ...
    def expand(self):
        ...
        next_state = move(self.state,action) #look at your own defined move function here above! Or should it 
                                   #be: 'self.state.move(action)'?
...
if __name__ == '__main__':
    ...
    sim_num = 3 #set number of rollouts to perform in each execution 
    ...
    def initialize_state():
        ...
        earliest1=[0]*(2*num_reqs +2)
        earliest1[0]=e[0]
        latest1=[0]*(2*num_reqs+2)
        latest1[2*num_reqs+1]=l[2*num_reqs+1] #=l[0] FC: ???? 
